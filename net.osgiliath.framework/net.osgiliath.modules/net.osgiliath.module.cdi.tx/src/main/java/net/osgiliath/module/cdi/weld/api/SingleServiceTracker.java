...
public final class SingleServiceTracker<T> implements ServiceListener {
  ...
  public void open() {
    if (open.compareAndSet(false, true)) {
      try {
        ...
        ctx.addServiceListener(this, finalFilterString);
        findMatchingReference(null);
      }
      catch (InvalidSyntaxException e) {
        log.error("unexpected error opening service listener", e);
   ...
   * 
   * @param original
   *          the original service reference.
   */
  private void findMatchingReference(ServiceReference original) {
    try {
      ...
      log.error("unexpected error getting servicereference", e);
      // this can never happen. (famous last words :)
    }
  }
  ...
  private boolean update(ServiceReference deadRef, ServiceReference newRef,
    ...
    if (newRef != null)
      lock = newRef;
    else if (deadRef != null)
      lock = deadRef;
    else
      lock = this;

    // This lock is here to ensure that no two threads can set the ref and
    // service
    // at the same time.
    synchronized (lock) {
      if (open.get()) {
