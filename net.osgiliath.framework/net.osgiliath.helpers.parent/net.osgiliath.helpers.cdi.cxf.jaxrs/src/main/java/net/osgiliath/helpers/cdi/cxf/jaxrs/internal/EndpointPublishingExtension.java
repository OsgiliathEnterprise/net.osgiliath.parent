...
public class EndpointPublishingExtension implements Extension {
   ...
   *          manager
   */
  public <X> void processPublishJaxRSServices(@Observes ProcessBean<X> process,
      BeanManager manager) {
    ...
    final List<Class> interfaces = new ArrayList<Class>(
        ClassUtils.getAllInterfaces(annotatedType.getJavaClass()));
    interfaces.addAll(ClassUtils.getAllSuperclasses(annotatedType
        .getJavaClass()));
    if (interfaces.isEmpty()) {
      process.addDefinitionError(new IllegalArgumentException(String.format(
    ...
    if (!isService) {
      process
          .addDefinitionError(new IllegalArgumentException(
              String
                  .format(
   ...
   *           didn't remember
   */
  private <X> void registerRSService(@Observes AfterDeploymentValidation event,
      BeanManager manager) throws ClassNotFoundException {
    ...
    for (Entry<String, JAXRSServerFactoryBean> facts : factories.entrySet()) {
      ...
      for (ProcessBean processBean : beansToProcess) {
        ...
        if (serviceURL.equals(facts.getKey())) {
          ...
          LOG.info("Creating CXF Endpoint for " + bean.getBeanClass().getName()
              + " " + "on service address: " + serviceURL);
        }
      }
  ...
  private void fillInterceptorList(
    ...
    for (Class clazz : toAddInterceptors) {
      ...
      if (!found) {
        throw new ClassNotFoundException("Interceptor " + clazz.getName()
  ...
  private JAXRSServerFactoryBean getFactory(String factoryId) {
    ...
    if (factory == null && CXFHelperActivator.getPlugin() != null
        && CXFHelperActivator.getPlugin().getJaxRSApplication() != null) {
      factory = RuntimeDelegate.getInstance().createEndpoint(
          CXFHelperActivator.getPlugin().getJaxRSApplication(),
      ...
      factory.setBus(bus);
      BindingFactoryManager manager = factory.getBus().getExtension(
